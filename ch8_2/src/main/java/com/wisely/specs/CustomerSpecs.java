...
public class CustomerSpecs {
	...
	public static <T> Specification<T> byAuto(final EntityManager entityManager, final T example) { //【1】定义一个返回值为 Specification<T> 的static方法。
		...
		return new Specification<T>() {
			...
			public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				List<Predicate> predicates = new ArrayList<>(); //			【3】定义Predicate列表 存储构造的 查询条件。
				
				EntityType<T> entity = entityManager.getMetamodel().entity(type);//	【4】实体的类型
				
				for (Attribute<T, ?> attr : entity.getDeclaredAttributes()) {//				【5】实体的属性
					Object attrValue = getValue(example, attr); //			【6】反射实体属性值
					if (attrValue != null) {
						if (attr.getJavaType() == String.class) { //7
							if (!StringUtils.isEmpty(attrValue)) { //8
								predicates.add(cb.like(root.get(attribute(entity, attr.getName(), String.class)),
										pattern((String) attrValue))); //				【 9】  构造  %like%
							}
						} else {
