...
//@Configuration
@EnableBatchProcessing		//必须开启对批处理的支持。
public class CsvBatchConfig {

	@Bean
	public ItemReader<Person> reader() throws Exception {
		FlatFileItemReader<Person> reader = new FlatFileItemReader<Person>(); //1	大文件读取
		reader.setResource(new ClassPathResource("people.csv")); //2
	        reader.setLineMapper(new DefaultLineMapper<Person>() {{ 				//3	每行数据映射到Person实体类中。
	            setLineTokenizer(new DelimitedLineTokenizer() {{
	                setNames(new String[] { "name","age", "nation" ,"address"});	//3	每行数据映射到Person实体类中。
	            }});
	...
	public Step step1(StepBuilderFactory stepBuilderFactory, ItemReader<Person> reader, ItemWriter<Person> writer,
		...
		return stepBuilderFactory
				...
				.<Person, Person>chunk(65000) //1 每批处理条数。
				.reader(reader) //2
